...
public interface BookingRepo extends JpaRepository<Booking, Long> {
    ...
    @Query("SELECT u FROM Booking u " +
            "WHERE (:id IS NULL OR u.user.id = :id) " +
            "AND (:idHour IS NULL OR u.idHour = :idHour)" +
            "AND (:idUser IS NULL OR u.user.id = :idUser) " +
            "AND (:status IS NULL OR u.status = :status) " +
            "AND :date IS NULL OR u.date = :end")
    List<Booking> checkLich(Long id,Long idHour, Long idUser, Status status,Date date);
}
